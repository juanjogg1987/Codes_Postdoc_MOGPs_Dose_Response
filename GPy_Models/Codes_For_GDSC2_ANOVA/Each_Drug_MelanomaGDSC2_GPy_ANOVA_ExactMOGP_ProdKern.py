...
for Nfold in range(nsplits,nsplits+1):
    ...
    for i in range(Yval_curve.shape[0]):
        ...
        if (res1 & res2).sum() > 0:
            ...
            aux_IC50 = x_dose_new[np.where(y_resp_interp==y_resp_interp.max())[0]][0]  # it has to be a float not an array to avoid bug
...
#
# final_path = '/data/ac1jjgg/Data_Marina/GPy_results/Codes_for_GDSC2_ANOVA/Drug'+config.drug_name+'_MelanomaGDSC2_ANOVA_ExactMOGP_ProdKern/'
# #final_path ='model_Drug'+config.drug_name+'_MelanomaGDSC2_ANOVA_ExactMOGP_ProdKern/'
