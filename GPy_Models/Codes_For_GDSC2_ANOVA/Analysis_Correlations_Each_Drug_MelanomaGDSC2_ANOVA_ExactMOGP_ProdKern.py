...
m_pred_all, v_pred_all = model.predict(Xtrain, full_cov=False)
Ntrain_raw = Xtrain.shape[0]//Ntasks
m_pred_curve_all = m_pred_all.reshape(Ntasks,Ntrain_raw).T.copy()
v_pred_curve_all = v_pred_all.reshape(Ntasks,Ntrain_raw).T.copy()
...
Feat_Pears_Data, Feat_Pval_Pears_Data, Feat_Spearm_Data,Feat_Pval_Spearm_Data = Compute_Correlations(Xtrain[0:Ntrain_raw,0:24].copy(),Yall.copy()) #m_pred_curve_all.copy()
...
df_KL_Ranking = pd.read_csv("/home/juanjo/Work_Postdoc/my_codes_postdoc/GPy_Models/Codes_For_GDSC2_ANOVA/Rankings_KLRelevance_MelanomaGDSC2/Drug"+config.drug_name+"_KLRelevance_MelanomaGDSC2_ANOVA_ProdKern_ExactMOGP.csv")
df_Pears_Ranking = pd.DataFrame(data=np.array(np.abs(Mean_Pears))[None,:],columns=list(Name_Features_Melanoma))

df_KL_Sorted = df_KL_Ranking[df_Pears_Ranking.columns]

plt.figure(1000)
plt.scatter(df_KL_Sorted,df_Pears_Ranking)

print(scipy.stats.pearsonr(df_KL_Sorted.values.flatten(),df_Pears_Ranking.values.flatten()))