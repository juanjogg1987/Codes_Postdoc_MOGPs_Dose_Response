...
for Nfold in range(nsplits,nsplits+1):
    ...
    model[:] = np.load('/home/juanjo/Work_Postdoc/my_codes_postdoc/GPy_Models/Codes_For_GDSC2_ANOVA/Best_Model_Drug' + config.drug_name + '_MelanomaGDSC2_GPy_ANOVA_ExactMOGP_ProdKern/m_' + m_trained + '.npy')
...
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"Below we prepare the data for Test over Drugs from GDSC1 with 9 Doses, i.e. 1036"
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

#_FOLDER_GDSC1 = "/home/ac1jjgg/Dataset_BRAF_NoReplica_ANOVA_Features/GDSC1/"
_FOLDER_GDSC1 = "/home/juanjo/Work_Postdoc/my_codes_postdoc/Dataset_BRAF_NoReplica_ANOVA_Features/GDSC1/"
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
df_train_No_MolecForm_GDSC1 = pd.read_csv(_FOLDER_GDSC1 + name_for_KLrelevance)  # Contain Train dataset prepared by Subhashini-Evelyn
df_ANOVA_feat_Names_GDSC1 = pd.read_csv(_FOLDER_GDSC1 + name_ANOVA_feat_file)  # Contain Feature Names used by ANOVA
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
df_train_GDSC1 = df_train_No_MolecForm_GDSC1[(df_train_No_MolecForm_GDSC1["DRUG_ID"]==int(config.drug_name))]
try:
    df_train_GDSC1 = df_train_GDSC1.drop(columns='Drug_Name')
except:
    pass
...
print(df_train_No_MolecForm_GDSC1.columns[start_pos_features])

""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"Below we use exactly the same features used for the training on GDSC2 their names are in Name_Features_Melanoma"
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
X_train_features_GDSC1_NonScaled = df_train_GDSC1[Name_Features_Melanoma].copy()
"Instead of using (MaxMin sclaer) scaler.transform we just extract the min and max and make the transformation"
"This is to directly use the features used for training GDSC2 Dose7"
scaler_sel_max = scaler.data_max_
scaler_sel_min = scaler.data_min_
Xall_GDSC1 = (X_train_features_GDSC1_NonScaled.values-scaler_sel_min)/(scaler_sel_max-scaler_sel_min)

"Below we select just 9 concentration since GDSC1 only has such a number"
y_train_drug_Dose9 = np.clip(df_train_GDSC1["norm_cells_" + str(1)].values[:, None], 1.0e-9, np.inf)
print(y_train_drug_Dose9.shape)
for i in range(2, Dnorm_cell+1):
    y_train_drug_Dose9 = np.concatenate(
        (y_train_drug_Dose9, np.clip(df_train_GDSC1["norm_cells_" + str(i)].values[:, None], 1.0e-9, np.inf)), 1)

print("Ytrain size: ", y_train_drug_Dose9.shape)

""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

params_4_sig_train_Dose9 = df_train_GDSC1["param_" + str(1)].values[:, None]
for i in range(2, 5):
    params_4_sig_train_Dose9 = np.concatenate(
        (params_4_sig_train_Dose9, df_train_GDSC1["param_" + str(i)].values[:, None]), 1)
...
x_real_dose_Dose9 = np.linspace(0.111111, 1, Dnorm_cell)  #Here is Dnorm_cell due to using GDSC1 that has 9 or 5 doses
...
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
# Compute Log(AUC)? R/ Not for Functional Random Forest Model
AUC_Dose9 = np.array(AUC_Dose9)[:,None]
IC50_Dose9 = np.array(IC50_Dose9)[:,None]
Emax_Dose9 = np.array(Emax_Dose9)[:,None]

""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"The Xtest_aux_Dose9 data should have labels of Ntask equal to the size of GDSC2, for instance if using Drug1036"
"then we should assign labels for 7 concentrations, that is because the models trained on GDSC2 presented"
"7 concentrations, so when we predict using the model we will obtain 7 predictions."

Xtest_aux_Dose9 = Xall_GDSC1.copy()
Ylabel_test_Dose9 = np.array([i * np.ones(Xtest_aux_Dose9.shape[0]) for i in range(Ntasks)]).flatten()[:, None]
Xtest_GDSC2_Dose9 = np.concatenate((np.tile(Xtest_aux_Dose9, (Ntasks, 1)), Ylabel_test_Dose9), 1)
Ytest_Dose9 = y_train_drug_Dose9.T.flatten().copy()[:, None]
...
m_pred_Dose9, v_pred_Dose9 = model.predict(Xtest_GDSC2_Dose9, full_cov=False)

Nval_Dose9 = Xtest_aux_Dose9.shape[0]

m_pred_curve_Dose9 = m_pred_Dose9.reshape(Ntasks, Nval_Dose9).T.copy()
v_pred_curve_Dose9 = v_pred_Dose9.reshape(Ntasks, Nval_Dose9).T.copy()

"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"Interpolation of predictions for GDSC1 for Dose9"
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
if Ntasks == 7:
    print("Remember This is for drugs in GDSC2 with 7 Concentrations")
    assert Ntasks == 7
...
x_dose_Dose9 = np.linspace(0.111111-difference, 1.0, Ntasks)  # Here 7 due to having 7 dose responses for GDSC2 prediction
x_dose_new_Dose9 = np.linspace(0.111111-difference, 1.0, 1000+239)
Ydose50_pred_Dose9 = []
IC50_pred_Dose9 = []
AUC_pred_Dose9 = []
Emax_pred_Dose9 = []
...
for i in range(m_pred_curve_Dose9.shape[0]):
    ...
    #y_resp_interp_Dose9 = f2(x_dose_new_Dose9)
    #std_upper_interp = f2_upper(x_dose_new_Dose9)
    #std_lower_interp = f2_lower(x_dose_new_Dose9)
    ...
    AUC_pred_Dose9.append(metrics.auc(x_dose_new_Dose9[239:], y_resp_interp_Dose9[239:])) #here from 239 is close to 0.111111
...
plt.plot(x_dose_new_Dose9, np.ones_like(x_dose_new_Dose9) * Emax_pred_Dose9[posy],'b--')  # Plot a horizontal line as Emax
plt.plot(x_dose_new_Dose9, np.ones_like(x_dose_new_Dose9) * Emax_Dose9[posy],'r--')  # Plot a horizontal line as Emax
plt.title(f"AUC Dose9 = {AUC_pred_Dose9[posy]}")
print(AUC_pred_Dose9[posy])

"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
Emax_abs_Dose9 = np.mean(np.abs(Emax_Dose9 - Emax_pred_Dose9))
AUC_abs_Dose9 = np.mean(np.abs(AUC_Dose9 - AUC_pred_Dose9))
IC50_MSE_Dose9 = np.mean((IC50_Dose9 - IC50_pred_Dose9) ** 2)
...
print("IC50 MSE:", IC50_MSE_Dose9)
print("AUC MAE:", AUC_abs_Dose9)
print("Emax MAE:", Emax_abs_Dose9)
...
pos_Actual_IC50 = IC50_Dose9 != 1.5
spear_corr_all, p_value_all = spearmanr(IC50_Dose9, IC50_pred_Dose9)
spear_corr_actualIC50, p_value_actual = spearmanr(IC50_Dose9[pos_Actual_IC50],IC50_pred_Dose9[pos_Actual_IC50])
print("Spearman_all Corr: ", spear_corr_all)
print("Spearman p-value: ", p_value_all)
print("Spearman_actualIC50 Corr: ", spear_corr_actualIC50)
print("Spearman p-value: ", p_value_actual)
...
"Some Box plots"
plt.figure(2000)
plt.boxplot(np.concatenate((Emax_Dose9,Emax_pred_Dose9),1))
plt.title("Emax")
plt.figure(3000)
plt.boxplot(np.concatenate((AUC_Dose9,AUC_pred_Dose9),1))
plt.title("AUC")
plt.figure(4000)
plt.boxplot(np.concatenate((IC50_Dose9,IC50_pred_Dose9),1))
#plt.boxplot(np.concatenate((IC50_Dose9[IC50_Dose9!=1.5][:,None],IC50_pred_Dose9[IC50_Dose9!=1.5][:,None]),1))
#plt.boxplot(np.concatenate((IC50_Dose9[IC50_Dose9==1.5][:,None],IC50_pred_Dose9[IC50_Dose9==1.5][:,None]),1))
plt.title("IC50")
...
FileName = "Drug"+config.drug_name+"_MelanomaGDSC2_ANOVA_ToPredict_MelanomaGDSC1_ExactMOGP_m"+m_trained
for pth in range(24):
    relevance = Utils_KLRelevance.KLRelevance_MOGP_GPy(train_x=Xtest_GDSC2_Dose9, model=model, delta=1.0e-4,which_p=pth,diag = False,Use_Cholesky = False, ToSave = True,FileName=FileName)
