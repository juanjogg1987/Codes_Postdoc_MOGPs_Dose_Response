...
class commandLine:
    def __init__(self):
        opts, args = getopt.getopt(sys.argv[1:], 'i:s:r:p:')
        # opts = dict(opts)
        # print(opts)
...
f.write("bash" + str(
    config.bash) + f" IC50_MSE={np.mean(IC50_MSE_AllFolds):0.5f}({np.std(IC50_MSE_AllFolds):0.5f}) NegLPD={np.mean(NegMLL_AllFolds):0.5f}({np.std(NegMLL_AllFolds):0.5f}) Spear_ActualIC50={np.mean(SpearActualIC50_AllFolds):0.5f}({np.std(SpearActualIC50_AllFolds):0.5f}) Spear_all={np.mean(Spearman_AllFolds):0.5f}({np.std(Spearman_AllFolds):0.5f}) test_MSE_IC50= {np.mean((Yval - IC50_pred) ** 2):0.5f} test_NegLPD={Val_NMLL:0.5f} \n")
f.close()

"The last model should have been trained over all dataset without splitting"

#final_path = '/data/ac1jjgg/Data_Marina/GPy_results/Codes_for_GDSC2/MelanomaGDSC2_SmallDataset_SOGP_IC50/'
final_path ='model_MelanomaGDSC2_SOGP_IC50/'
