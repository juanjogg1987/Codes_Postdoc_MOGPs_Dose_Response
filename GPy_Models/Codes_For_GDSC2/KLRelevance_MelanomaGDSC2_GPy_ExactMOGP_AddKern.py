...
"Best model: All_Drugs_MelanomaGDSC2_GPy_ExactMOGP_AddKern.py -i 1500 -s 3.0000 -k 6 -w 0.1000 -r 1010 -d 5 -p 376"
class commandLine:
    def __init__(self):
        opts, args = getopt.getopt(sys.argv[1:], 'i:s:k:w:r:d:p:e:')
        # opts = dict(opts)
        # print(opts)
        ...
        for op, arg in opts:
            ...
            if op == '-d':  # split of the additive kernel
                self.split_dim = arg
            if op == '-e':  # feature to ckeck KLRelevance
                self.feature = arg
...
for Nfold in range(nsplits,nsplits+1):
    ...
    mykern = GPy.kern.RBF(size_dims, active_dims=list(np.arange(0, size_dims)))
    for i in range(1, split_dim):
        if i != (split_dim - 1):
            mykern = mykern + GPy.kern.RBF(size_dims, active_dims=list(np.arange(size_dims * i, size_dims * i + size_dims)))
            print(list(np.arange(size_dims*i,size_dims*i+size_dims)))
        else:
            last_dims = Dim - size_dims * i
            mykern = mykern + GPy.kern.RBF(last_dims, active_dims=list(np.arange(size_dims * i, Dim)))
            print(list(np.arange(size_dims * i, Dim)))
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    ...
    model[:] = np.load('/home/juanjo/Work_Postdoc/my_codes_postdoc/GPy_Models/Codes_For_GDSC2/Best_Model_MelanomaGDSC2_GPy_ExactMOGP_AddKern/m_' + m_trained + '.npy')
    # model[:] = np.load('/home/ac1jjgg/MOGP_GPy/Codes_for_GDSC2/Best_Model_MelanomaGDSC2_GPy_ExactMOGP/m_' + m_trained + '.npy')
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
