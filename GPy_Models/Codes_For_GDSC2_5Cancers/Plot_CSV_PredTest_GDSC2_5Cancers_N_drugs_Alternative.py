...
for sel_cancer in cancers:
    ...
    def plot_Nth_dose(sel_cancer,axs,sel_dose,Num_cells,AE_per_dose_Ncells,MAE_per_dose_Ncells,Responsive = True,my_ylim=None,my_title=None,force_title=False):
        ...
        if Responsive is True:
            ...
            axs[sel_cancer].boxplot(AE_per_Nthdose_Ncells, medianprops=dict(color="orangered", linewidth=1.8),
                                              positions=Total_Ncell, widths=25.0, notch=True,
    ...
    def plot_benchmark(axs, loc, N_Cells_lin, data,alpha = 0.5, Responsive = True):
        if Responsive is True:
            ...
            axs[0, 0].legend(handles=[line_mean,line_averMAE,line_Q3,line_Q2,line_Q1],loc='upper right', bbox_to_anchor=(2.01, 1.3),ncol=5, fancybox=True, shadow=True)
        else:
            line_Q1, = axs[loc[0]].plot(N_Cells_lin, np.percentile(data, 25) * np.ones_like(N_Cells_lin), 'm--',linewidth=1.1, alpha=alpha, label='Benchmark Q1')
            line_Q2, = axs[loc[0]].plot(N_Cells_lin, np.percentile(data, 50) * np.ones_like(N_Cells_lin), 'm',linewidth=1.1, alpha=alpha, label='Benchmark Q2')
            line_Q3, = axs[loc[0]].plot(N_Cells_lin, np.percentile(data, 75) * np.ones_like(N_Cells_lin), 'm--',linewidth=1.1, alpha=alpha, label='Benchmark Q3')
            line_mean, = axs[loc[0]].plot(N_Cells_lin, np.mean(data) * np.ones_like(N_Cells_lin), 'green',linewidth=1.1, alpha=alpha, label='Benchmark Mean')
            # line_median, = axs[loc[0], loc[1]].plot(1000*N_Cells_lin, np.mean(data) * np.ones_like(N_Cells_lin), 'm',alpha=1.0, label='Boxplot Median')
