...
figaux, axsaux = plt.subplots(5, 3,figsize = (13,19))   #(10,16)
...
for sel_cancer in cancers:
    ...
    def plot_Nth_dose(indx_plot,axs,sel_dose,Num_cells,AE_per_dose_Ncells,MAE_per_dose_Ncells,mycolor = ['blue','blue'],Responsive = True,my_ylim=None,my_title=None,force_title=False):
        ...
        if Responsive is True:
            ...
            line3 = axs[Nth_dose,indx_plot ].plot(New_X_per_seed, MAE_per_seed, '.', color=mycolor[0], alpha=0.5,label='Mean-Error per seed')
            line2, = axs[Nth_dose,indx_plot].plot(New_X, f_Mean_MAE_per_Nthdose_Ncells,'-',color=mycolor[1],linewidth=mylinewidth,label = 'Avg. Mean-Error ± Std')
            if my_ylim is None:
                axs[Nth_dose,indx_plot].set_ylim([-0.01,0.27])
            else:
                axs[Nth_dose,indx_plot].set_ylim(my_ylim)
            axs[Nth_dose,indx_plot].grid(False)
            if indx_plot==0 or force_title==True:
                ...
                if my_title is None:
                    axs[Nth_dose,indx_plot].set_title(f"Dose {Nth_dose+1}",fontsize=15)
                else:
                    axs[Nth_dose,indx_plot].set_title(my_title)
        else:
            ...
            line3 = axs[indx_plot].plot(New_X_per_seed, MAE_per_seed, '.', color=mycolor[0], alpha=0.5,label='Mean-Error per seed')
    ...
    def plot_benchmark(axs, loc, N_Cells_lin, data,alpha = 0.5, Responsive = True):
        ...
        if Responsive is True:
            ...
            line_Q2, = axs[loc[1], loc[0]].plot(N_Cells_lin, np.percentile(data, 50) * np.ones_like(N_Cells_lin), '--',color=mycolor,linewidth=1.3,alpha=alpha,label='BERK Q2')
            ...
            line_mean, = axs[loc[1], loc[0]].plot(N_Cells_lin, np.mean(data) * np.ones_like(N_Cells_lin), color=mycolor,linewidth=1.3,alpha=alpha,label='BERK Mean-Error')
...
cancer_name_plot = {0:'BRCA (Error)',1:'COREAD (Error)',2:'LUAD (Error)',3:'SKCM (Error)',4:'SCLC (Error)'}
cancer_name_plot_abs = {0:'BRCA',1:'COREAD',2:'LUAD',3:'SKCM',4:'SCLC'}

for i in range(5):
    axs_all[0][i, 0].set_title('', fontsize=14, x=1.5)
    axs_all[0][i, 1].set_title('', fontsize=14, x=1.5)
    axs_all[0][i, 2].set_title('', fontsize=14, x=1.5)
    axs_all[0][i,0].set_xlabel('', fontsize=14,x=1.5)
    axs_all[0][i,1].set_xlabel('', fontsize=14, x=1.5)
    axs_all[0][i,2].set_xlabel('', fontsize=14, x=1.5)

axs_all[0][0,0].set_title('AUC (MAE)', fontsize=14,x=0.5)
axs_all[0][0,1].set_title('Emax (MAE)', fontsize=14, x=0.5)
axs_all[0][0,2].set_title('IC50 (MSE)', fontsize=14, x=0.5)
...
fig_all[0].tight_layout(pad=6.0, w_pad=0.0, h_pad=1.0)

#fig_all[0].tight_layout(w_pad=0.7)
#fig_all[0].tight_layout(h_pad=0.39)

for i in range(5):
    fig_all[0].supxlabel('\nNumber of dose response curves in training',fontsize=17,x=0.5,y=0.02)
    #if i != 3:
    myindx= sel_col[i]-1
    axs_all[0][myindx,0].set_ylabel(cancer_name_plot_abs[i],fontsize=14)
    axs_all[0][myindx,1].set_ylabel("")
    axs_all[0][myindx,2].set_ylabel("")
...
def plot_ChenYurui_BenchMark(All_Metrics_ordered,sel_c,which_metric = 'AUC',color=None):
    ...
    axs_all[0][sel_c,d_loc[which_metric]].plot(New_X_per_seed, dict_metric[which_metric], '.', color=color, alpha=0.25,label='Mean-Error per seed')  #
    axs_all[0][sel_c,d_loc[which_metric]].plot(New_X, f_Mean_MAE_per_Nthdose_Ncells, '-.', color=color, linewidth=2.0, alpha=1.0,
...
selcolor = {'DeepCDR':'#8B4513','GraphDRP':'pink','NeRD':'magenta','LR_results/cna/LR_result':'red',
            'LR_results/mut/LR_result':'orange','LR_results/meth/LR_result':'cyan','Enet_results/cna/ENET_result':'yellow',
            'Enet_results/mut/ENET_result':'purple','Enet_results/meth/ENET_result':'#b2df8a','Enet_results/AllFeat/ENET_result':'black'}
Names_BK_model = ['DeepCDR','GraphDRP','NeRD','LR_results/cna/LR_result','LR_results/mut/LR_result',
                  'LR_results/meth/LR_result','Enet_results/cna/ENET_result','Enet_results/mut/ENET_result',
                  'Enet_results/meth/ENET_result','Enet_results/AllFeat/ENET_result']
...
#                        ["_nolegend_"]*5+["ME-seed (LR-cna)","Avg.±std (LR-cna)"]+["_nolegend_"]*5+["ME-seed (LR-mut)","Avg.±std (LR-mut)"]+["_nolegend_"]*5+["ME-seed (LR-meth)","Avg.±std (LR-meth)"]+\
#                        ["_nolegend_"]*5+["ME-seed (Enet-cna)","Avg.±std (Enet-cna)"]+["_nolegend_"]*5+["ME-seed (Enet-mut)","Avg.±std (Enet-mut)"]+["_nolegend_"]*5+["ME-seed (Enet-meth)","Avg.±std (Enet-meth)"]+\
#                        ["_nolegend_"]*5+["ME-seed (Enet-Mul)","Avg.±std (Enet-Mul)"],loc='upper right',bbox_to_anchor=(3.7, 1.35), ncol=8, fancybox=True, shadow=True,labelspacing=-0.15, columnspacing=0.25)

# axs_all[0][0,2].legend(["_nolegend_"]*5+["ME-seed (MOGP)","Avg-ME (MOGP)"]+["_nolegend_"]*5+["ME-seed (SRMF)","Avg-ME (SRMF)"]+["Median (BERK)","Mean (BERK)"]+["_nolegend_"]*5+\
#                        ["ME-seed (DeepCDR)","Avg-ME (DeepCDR)"]+["_nolegend_"]*5+["ME-seed (GraphDRP)","Avg-ME (GraphDRP)"]+["_nolegend_"]*5+["ME-seed (NeRD)","Avg-ME (NeRD)"]+\
#                        ["_nolegend_"]*5+["ME-seed (LR-cna)","Avg-ME (LR-cna)"]+["_nolegend_"]*5+["ME-seed (LR-mut)","Avg-ME (LR-mut)"]+["_nolegend_"]*5+["ME-seed (LR-meth)","Avg-ME (LR-meth)"]+\
#                        ["_nolegend_"]*5+["ME-seed (Enet-cna)","Avg-ME (Enet-cna)"]+["_nolegend_"]*5+["ME-seed (Enet-mut)","Avg-ME (Enet-mut)"]+["_nolegend_"]*5+["ME-seed (Enet-meth)","Avg-ME (Enet-meth)"]+\
#                        ["_nolegend_"]*5+["ME-seed (Enet-Mul)","Avg-ME (Enet-Mul)"],loc='upper right',bbox_to_anchor=(3.7, 1.35), ncol=8, fancybox=True, shadow=True,labelspacing=-0.1, columnspacing=0.28)
