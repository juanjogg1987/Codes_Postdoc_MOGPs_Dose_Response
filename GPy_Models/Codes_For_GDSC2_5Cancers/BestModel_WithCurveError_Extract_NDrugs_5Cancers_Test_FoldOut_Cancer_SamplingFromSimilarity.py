...
for Nfold in range(nsplits,nsplits+1):
    ...
    MAE_per_Dose = np.mean(np.abs(m_pred_curve - Yval_curve), 0)
    ...
    print("MAE per Dose:",MAE_per_Dose)
    ...
    IC50_Resp_MSE = np.mean((IC50_val[pos_Actual_IC50]- IC50_pred[pos_Actual_IC50])**2)
    IC50_NoResp_MSE = np.mean((IC50_val[pos_No_IC50] - IC50_pred[pos_No_IC50]) ** 2)

    print("MSE IC50 Reponsive:",IC50_Resp_MSE)
    print("MSE IC50 Non-Reponsive:", IC50_NoResp_MSE)
...
plt.close('all')
plt.hist(Yall[:,-1],10)
plt.hist(m_pred_curve[:,-1],10)
plt.hist(Yval_curve[:,-1],10)
