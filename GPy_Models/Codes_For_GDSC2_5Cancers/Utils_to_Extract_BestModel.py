...
def model_pred_test_csv(path_to_read_bash,path_to_model):
    df_bash_data = pd.read_csv(path_to_read_bash, header=None, skiprows=6)
    params_vect = df_bash_data[0].values[0][121:].split(" ")
    for k, op in enumerate(params_vect):
        if op == '-i':
    ...
    # df_4Cancers_test.insert(25, "Emax_MOGP", Emax_pred)  #This is useful to insert in the particular location
