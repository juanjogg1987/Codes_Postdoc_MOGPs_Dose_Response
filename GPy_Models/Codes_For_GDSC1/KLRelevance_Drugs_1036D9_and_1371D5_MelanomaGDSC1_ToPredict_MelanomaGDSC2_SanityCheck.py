...
"BestModel: Drugs_1036Dose9_and_1371Dose5_MelanomaGDSC1_ToPredict_MelanomaGDSC2_SanityCheck.py -i 1500 -s 3.0000 -k 2 -w 0.0100 -r 1017 -p 622 -e %d"
class commandLine:
    def __init__(self):
        ...
        self.which_seed = 1017    #change seed to initialise the hyper-parameters
        self.rank = 2
        self.scale = 3.0
        self.weight = 0.01
        self.bash = "622"
...
for Nfold in range(nsplits,nsplits+1):
    ...
    #model[:] = np.load('/home/juanjo/Work_Postdoc/my_codes_postdoc/GPy_Models/Codes_For_GDSC1/Best_Model_Drugs_1036D9_and_1371D5_MelanomaGDSC1_GPy_ExactMOGP_ToPredict_MelanomaGDSC2_SanityCheck/m_' + m_trained + '.npy')
    model[:] = np.load('/home/ac1jjgg/MOGP_GPy/Codes_for_GDSC1/Best_Model_Drugs_1036D9_and_1371D5_MelanomaGDSC1_GPy_ExactMOGP_ToPredict_MelanomaGDSC2_SanityCheck/m_' + m_trained + '.npy')
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
for i in range(params_4_sig_train_Dose7.shape[0]):
    "Below we locate the Y values for the ajusted region"
    Ydose_res_Dose7.append(sigmoid_4_param(x_lin_tile_Dose7_Adjust[i, :], *params_4_sig_train_Dose7[i, :]))
    AUC_Dose7.append(metrics.auc(x_lin_tile_Dose7[i, :], Ydose_res_Dose7[i]))  #Here the AUC is from 0.111111
...
ftest= open("Metrics_Test_GDSC2.txt","w+")
...
X_aux_GDSC1 = Xall.copy()
Ylabel_GDSC1 = np.array([i * np.ones(Xall.shape[0]) for i in range(Ntasks9)]).flatten()[:, None]
X_GDSC1 = np.concatenate((np.tile(X_aux_GDSC1, (Ntasks9, 1)), Ylabel_GDSC1), 1)

relevance_GDSC1 = Utils_KLRelevance.KLRelevance_MOGP_GPy(train_x=X_GDSC1, model=model, delta=1.0e-6,which_p=int(config.feature),diag = False,Use_Cholesky = False, ToSave = True, FileName="MelanomaGDSC1")
print("For GDSC1",np.mean(relevance_GDSC1,0))

relevance_GDSC2 = Utils_KLRelevance.KLRelevance_MOGP_GPy(train_x=Xtest_GDSC2, model=model, delta=1.0e-6,which_p=int(config.feature),diag = False,Use_Cholesky = False, ToSave = True,FileName="MelanomaGDSC2")
print("For GDSC2",np.mean(relevance_GDSC2,0))
