...
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"#This file only contains drugs 1061 and 1036, but here with 9 concentrations"
name_for_KLrelevance = 'GDSC1_melanoma_BRAF_9conc.csv'

df_train_No_MolecForm = pd.read_csv(_FOLDER + name_for_KLrelevance)  # Contain Train dataset prepared by Subhashini-Evelyn
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
df_train_No_MolecForm = df_train_No_MolecForm[(df_train_No_MolecForm["DRUG_ID"]==1036)]
try:
...
x_real_dose = np.linspace(0.111111, 1, 9)  #Here is 9 due to using GDSC1 for Drug1061 that has 9 doses
...
"BestModel: Drug1036_MelanomaGDSC1_GPy_ExactMOGP.py -i 1500 -s 1.0000 -k 4 -w 1.0000 -r 1012 -p 291"
class commandLine:
    def __init__(self):
        ...
        self.which_seed = 1012    #change seed to initialise the hyper-parameters
        self.rank = 4
        self.scale = 1.0
        self.weight = 1.0
        self.bash = "291"
...
config = commandLine()
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"The indexes below are 1 with response (4) and 1 without reponse (45 i.e., IC50=1.5)"

index_Test = np.array([4,45])
...
for Nfold in range(nsplits,nsplits+1):
    ...
    kern = GPy.kern.RBF(Dim, active_dims=list(np.arange(0, Dim))) ** GPy.kern.Coregionalize(1,output_dim=Ntasks,rank=rank)
    kern.rbf.lengthscale =  float(config.scale)* np.sqrt(Dim) * np.random.rand()
    #kern.rbf.lengthscale = float(config.scale) * np.sqrt(Dim) * np.random.rand(Dim)
    #kern.rbf.lengthscale = float(config.scale) * np.ones(Dim)
    ...
    #model.optimize(optimizer='lbfgsb',messages=True,max_iters=10)
    ...
    #model[:] = np.load('/home/juanjo/Work_Postdoc/my_codes_postdoc/GPy_Models/Codes_For_GDSC1/Best_Model_Drug1036_MelanomaGDSC1_GPy_ExactMOGP/m_' + m_trained + '.npy')
    model[:] = np.load('/home/ac1jjgg/MOGP_GPy/Codes_for_GDSC1/Best_Model_Drug1036_MelanomaGDSC1_GPy_ExactMOGP/m_' + m_trained + '.npy')
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    ...
    x_dose = np.linspace(0.111111, 1.0, 9)  #9 doses here
...
relevance = Utils_KLRelevance.KLRelevance_MOGP_GPy(train_x=Xtrain, model=model, delta=1.0e-4,which_p=int(config.feature),diag = False,Use_Cholesky = False, ToSave = True)
