...
class BMKMOGaussianProcess(nn.Module):
    ...
    def forward(self,xS, DrugC_new = None,idx_predict = None,NDomain_sel=0,noiseless = True):
        if self.Train_mode:
            ...
            if DrugC_new is None:
                ...
                DrugC_new = torch.Tensor(DrugC_new)[:, None]
            NewDouts = DrugC_new.shape[0]
            "Below idx_predict has to be a list of xS.shape[0] to associate what domains to predict per data"
            "if xS is M x P, the idx_predict is of length M"
            "For instance if idx_predict = [0,0,1,2,3], then the prediction would be for such domains"
            if idx_predict is None:
                #NDomain_sel = 0
                print(f"by default we would predict for domain {NDomain_sel}. Set idx_predict as per the domains to predict")
                print("For instance if idx_predict = [0,0,1,2,3], then the prediction would be for such domains")
                print("we would expect xS to be of shape[0] equal to length of idx_predict")
                idxS_new = [NDomain_sel] * xS.shape[0] * NewDouts  # Here xS is the new input value for which we want to pred
            else:
                idxS_new = idx_predict * NewDouts
...
def bypass_params(model_trained,model_cv):
    ...
    for i_kern in range(model_trained.TLCovariance.kernels.__len__()):
        ...
        model_cv.TLCovariance.kernels[i_kern].adq = model_trained.TLCovariance.kernels[i_kern].adq.clone()
...
for Nfold in range(0,nsplits):
    ...
    idx_ST_train_cv = list(idx_ST_numpy[train_ind])
    idx_ST_val_cv = list(idx_ST_numpy[val_ind])
    ...
    model_cv = BMKMOGaussianProcess(xST_train_cv, yST_train_cv, idxS=idx_ST_train_cv, DrugC=DrugC, NDomains=NDomains)

    bypass_params(model, model_cv)  #Here we bypass the fitted parameters from the MOGP trained over all data
    myTrain(model_cv, xST_train_cv, yST_train_cv, myLr=1e-6, Niter=1) #Here we could refine hyper-params a bit if wished
    ...
    with torch.no_grad():
        ...
        mpred_cv, Cpred_cv = model_cv(xST_val_cv, DrugC_new=DrugCtoPred_cv, idx_predict=idx_ST_val_cv,noiseless=False)
