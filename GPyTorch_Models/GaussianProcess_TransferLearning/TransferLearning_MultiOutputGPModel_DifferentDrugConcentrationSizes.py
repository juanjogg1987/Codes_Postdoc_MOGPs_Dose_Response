...
class TLMOGaussianProcess(nn.Module):
    ...
    def __init__(self,xT,yT,xS,yS,idxS,DrugC_T,DrugC_S,NDomains):
        ...
        self.xT = torch.kron(torch.ones(self.Douts_T, 1),xT)  #This is to replicate xT as per the D number of outputs yT
        self.xS = torch.kron(torch.ones(self.Douts_S, 1),xS)  #This is to replicate xS as per the D number of outputs yS
    ...
    def forward(self,xT, DrugC_new = None,noiseless = True):
        if self.Train_mode:
            xT = torch.kron(torch.ones(self.Douts_T, 1), xT)
...
y2 = torch.cat([y2,0.5*y2**1+0.5*(y2**3),-0.4*y2],1)
Many_y2 = torch.cat([Many_y2,0.5*Many_y2**1+0.5*(Many_y2**3),-0.4*Many_y2],1)
...
NDomains = 3
DrugC_S = [0.1,0.5]
DrugC_T = [0.1,0.25,0.5]
assert DrugC_S.__len__() == y0.shape[1] and DrugC_S.__len__() == y1.shape[1] and DrugC_T.__len__() == y2.shape[1]
model = TLMOGaussianProcess(x2,y2,train_xS,train_yS,idxS=idx0+idx1,DrugC_T=DrugC_T,DrugC_S=DrugC_S,NDomains=NDomains)
plt.plot(Many_x2,Many_y2[:,0],'y.',alpha=0.5)
plt.plot(Many_x2,Many_y2[:,1],'m.',alpha=0.5)
plt.plot(Many_x2,Many_y2[:,2],'c.',alpha=0.5)
...
with torch.no_grad():
    ...
    mpred, Cpred = model(x_test,DrugC_new = [DrugC_T[sel_concentr]],noiseless=True)
