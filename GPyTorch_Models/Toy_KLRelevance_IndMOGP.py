...
if which_toy =="toy1":
    ...
    #Xtrain = np.concatenate((x1[:,None],x2[:,None],x4[:,None],x3[:,None]),1)
    #Xval = np.concatenate((x1_val[:,None],x2_val[:,None],x4_val[:,None],x3_val[:,None]),1)
    #Xtrain = x1[:,None].copy()
    ...
    Xtrain = np.concatenate((x1[:, None], x2[:, None], x3[:, None], x4[:, None], x5[:, None]), 1)
    Xval = np.concatenate((x1_val[:, None], x2_val[:, None], x3_val[:, None], x4_val[:, None], x5_val[:, None]), 1)
    #Xtrain = np.concatenate((x2[:,None],x4[:,None]),1)
    #Xval = np.concatenate((x2_val[:,None],x4_val[:,None]),1)
    #Xtrain = x1[:,None].copy()
    #Xval = x1_val[:, None].copy()
    """"""""""""""""""""""""""""""""""""""""""""""""""
    """"""""""""""""""""""""""""""""""""""""""""""""""
...
for task in range(num_tasks):
    ...
    plt.title(f'Task {task + 1} Independent MOGP')
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
