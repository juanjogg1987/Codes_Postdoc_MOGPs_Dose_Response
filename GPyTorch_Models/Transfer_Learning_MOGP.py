...
class TL_GPModel(gpytorch.models.ExactGP):
    ...
    def forward(self,x):
        ...
        mean_x = KTS.matmul(CSS.inv_matmul(YS)).reshape(self.num_doses,-1).T  # KTS @ CSS_i @ YS
...
y1_ax.plot(test_x,lower[:, 0].numpy(), 'c')
y1_ax.plot(test_x,upper[:, 0].numpy(), 'c')
# Shade in confidence
...
y2_ax.plot(test_x,lower[:, 1].numpy(), 'c')
y2_ax.plot(test_x,upper[:, 1].numpy(), 'c')
# Shade in confidence
