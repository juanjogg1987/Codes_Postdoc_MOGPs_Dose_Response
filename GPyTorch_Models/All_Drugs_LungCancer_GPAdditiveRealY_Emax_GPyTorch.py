...
plt.plot(x_real_dose[8:9],Emax[posy],'rx')
plt.title(f"Emax = {Emax[posy]}")
print(Emax[posy])

"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
apply = "log"  #This means we will
Emax = np.array(Emax)
if apply == "log":
...
if apply=="log":  #Here it means we reverse the log by applying exp
    ...
    Emax_exp = np.exp(Emax)

saturated_mean = mean_pred_exp.copy()
saturated_mean[saturated_mean>1.0] = 1.0

Emax_Y_Val = Emax_exp[val_id]
print("Saturated Emax MAE:",np.mean(np.abs(Emax_Y_Val-saturated_mean)))
print("Emax MAE:",np.mean(np.abs(Emax_Y_Val-mean_pred_exp)))
print("Saturated Emax MedAE:",np.median(np.abs(Emax_Y_Val-saturated_mean)))
print("Emax MedAE:",np.median(np.abs(Emax_Y_Val-mean_pred_exp)))
...
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
MAE_Emax = np.mean(np.abs(Emax_Y_Val-saturated_mean))
MedAE_Emax = np.median(np.abs(Emax_Y_Val-saturated_mean))

f= open("Metrics.txt","a+")
f.write("bash"+str(config.bash)+f" MAE_Emax={MAE_Emax:0.4f} MEdAE_Emax={MedAE_Emax:0.4f}\n")
f.close()
#
...
#
#final_path = '/data/ac1jjgg/Data_Marina/GPyTorch_results/LungCancer_GPAdditiveCorrectRealY_NoLogEmax/'
final_path ='model_LungCancer_Emax/'
