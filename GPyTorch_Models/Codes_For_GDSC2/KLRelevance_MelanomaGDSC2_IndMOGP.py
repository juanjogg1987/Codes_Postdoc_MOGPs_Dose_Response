...
class commandLine:
    def __init__(self):
        ...
        # opts = dict(opts)
        # print(opts)
        self.N_iter_epoch = 1800    #number of iterations
        self.which_seed = 1019    #change seed to initialise the hyper-parameters
        self.scale = 0.01
        self.split_dim = 15
        self.feature = 0
        self.bash = "143"
...
for Nfold in range(nsplits,nsplits+1):
    ...
    class BatchIndependentMultitaskGPModel(gpytorch.models.ExactGP):
        def __init__(self, train_x, train_y, likelihood):
            ...
            mykern = gpytorch.kernels.LinearKernel(active_dims=torch.tensor(list(np.arange(0, size_dims))),
                                                   ...
                                                   batch_shape=torch.Size([num_tasks])) + gpytorch.kernels.RBFKernel(
                active_dims=torch.tensor(list(np.arange(0,size_dims))),
                ard_num_dims=size_dims, batch_shape=torch.Size([num_tasks]))
            for i in range(1, split_dim):
                if i != (split_dim - 1):
                    mykern = mykern + gpytorch.kernels.LinearKernel(
                        ...
                        ard_num_dims=size_dims, batch_shape=torch.Size([num_tasks])) + gpytorch.kernels.RBFKernel(
                        active_dims=torch.tensor(list(np.arange(size_dims * i, size_dims * i + size_dims))),
                        ard_num_dims=size_dims, batch_shape=torch.Size([num_tasks]))
                    ...
                    mykern = mykern + gpytorch.kernels.LinearKernel(
                        active_dims=torch.tensor(list(np.arange(size_dims * i, Dim))), ard_num_dims=last_dims,
                        batch_shape=torch.Size([num_tasks])) + gpytorch.kernels.RBFKernel(
                        active_dims=torch.tensor(list(np.arange(size_dims * i, Dim))), ard_num_dims=last_dims,
                        batch_shape=torch.Size([num_tasks]))
                    # print(torch.tensor(list(np.arange(size_dims*i,Dim))))

            self.covar_module = gpytorch.kernels.ScaleKernel(mykern,batch_shape=torch.Size([num_tasks]))
    ...
    for i in range(split_dim):
        d1, d3,d2 = model.covar_module.base_kernel.kernels[2 * i + 1].lengthscale.size()
        mylengthscale = float(config.scale) * np.sqrt(Dim) * np.random.rand(d1,d3, d2)  #d3 is always 1
    ...
    state_dict = torch.load('/home/juanjo/Work_Postdoc/my_codes_postdoc/GPyTorch_Models/Codes_For_GDSC2/Best_Model_MelanomaGDSC2_IndMOGP/m_' + m_trained + '.pth')
    # state_dict = torch.load("/home/ac1jjgg/MOGP_GPyTorch/Codes_for_GDSC2/Dataset_BRAF/Best_Model_MelanomaGDSC2/" +'m_'+ m_trained + '.pth')
    ...
    for i in range(val_y.numpy().shape[0]):
        ...
        if (res1 & res2).sum() > 0:
            ...
            aux_IC50 = x_dose_new[np.where(y_resp_interp[i].max())[0]][0]  # it has to be a float not an array to avoid bug
...
# ftest.write("bash"+str(config.bash)+f" Med_MSE={Med_MSE:0.5f} Mean_MSE={Mean_MSE:0.5f} NegMLL={Val_NMLL:0.5f} IC50_MSE={IC50_MSE:0.5f} Spear_ActualIC50={spear_corr_actualIC50:0.5f} Spear_all={spear_corr_all:0.5f} AUC_abs={AUC_abs:0.5f} Emax_abs ={Emax_abs:0.5f} \n")
# ftest.close()
...
#
# f= open("Relevance_MelanomaGDSC2.txt","a+")
# f.write(f"{which_p}")
# for n in range(N):
#     f.write(",")
