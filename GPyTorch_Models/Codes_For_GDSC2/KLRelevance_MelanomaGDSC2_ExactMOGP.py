...
for Nfold in range(nsplits,nsplits+1):
    ...
    class MultitaskGPModel(gpytorch.models.ExactGP):
        def __init__(self, train_x, train_y, likelihood):
            ...
            mykern = gpytorch.kernels.LinearKernel(active_dims=torch.tensor(list(np.arange(0, size_dims))),
                                                   ard_num_dims=size_dims) + gpytorch.kernels.RBFKernel(active_dims=torch.tensor(
                                                                                                               list(
                                                                                                                   np.arange(
                                                                                                                       0,
                                                                                                                       size_dims))),
    ...
    state_dict = torch.load('/home/juanjo/Work_Postdoc/my_codes_postdoc/GPyTorch_Models/Codes_For_GDSC2/Best_Model_MelanomaGDSC2_ExactMOGP/m_' + m_trained + '.pth')
