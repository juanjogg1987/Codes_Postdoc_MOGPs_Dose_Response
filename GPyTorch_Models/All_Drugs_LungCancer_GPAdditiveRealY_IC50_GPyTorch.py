...
class commandLine:
    def __init__(self):
        opts, args = getopt.getopt(sys.argv[1:], 'b:m:i:r:d:p:f:')
        # opts = dict(opts)
        # print(opts)
...
class GPModel(ApproximateGP):
    def __init__(self, inducing_points):
        ...
        mykern = gpytorch.kernels.MaternKernel(nu=0.5, active_dims=torch.tensor(list(np.arange(0, size_dims))),
        ...
        for i in range(1, split_dim):
            if i != (split_dim - 1):
                ...
                mykern = mykern + gpytorch.kernels.MaternKernel(nu=0.5, active_dims=torch.tensor(
                    list(np.arange(size_dims * i, size_dims * i + size_dims))), ard_num_dims=size_dims)
                ...
                mykern = mykern + gpytorch.kernels.MaternKernel(nu=0.5, active_dims=torch.tensor(
                    list(np.arange(size_dims * i, Dim))), ard_num_dims=last_dims)
...
spear_corr, p_value = spearmanr(IC50_Y_Val, saturated_mean)
...
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
MSE_IC50 = np.mean((IC50_Y_Val-saturated_mean)**2)
MSE_IC50_Affect = np.mean((IC50_Y_Val[pos_Affect]-saturated_mean[pos_Affect])**2)
MSE_IC50_NonAffect = np.mean((IC50_Y_Val[pos_NonAffect]-saturated_mean[pos_NonAffect])**2)
f= open("Metrics.txt","a+")
f.write("bash"+str(config.bash)+f" MSE_IC50={MSE_IC50:0.4f} MSE_IC50_Affect={MSE_IC50_Affect:0.4f} MSE_IC50_NonAffect={MSE_IC50_NonAffect:0.4f} SpearCorr={spear_corr:0.4f} spear_p-val={p_value:0.4f}\n")
f.close()
...
#final_path = '/data/ac1jjgg/Data_Marina/GPyTorch_results/LungCancer_GPAdditiveCorrectRealY_IC50/'
final_path ='model_LungCancer_IC50/'
